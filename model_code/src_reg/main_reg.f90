...
!print*,t
  ...
  if (data_assim) then
   ...
   ! soil state updating
   if ((obs(t,1).gt.0)) then
    do e = 1,Ne
     X(1) = state(t,e)%smc(1)
     X(2) = obs(t,1)
!     X(3) = 1
     state(t,e)%smc(1) = B(1)*X(1) + B(2)*X(2) !+ B(3)*X(3)

!print*,X
!print*,B
!print*,state(t,e)%smc(1)
...
! real, parameter :: sig_veg = 0.01
 ...
 ! soil moisture
 state%smc(1)  = state%smc(1) +eta(1)*0.006
 state%sh2o(1) = state%sh2o(1)+eta(1)*0.006

 state%smc(2)  = state%smc(2) +eta(2)*0.00011
 state%sh2o(2) = state%sh2o(2)+eta(2)*0.00011

 state%smc(3)  = state%smc(3) +eta(3)*0.0006
 state%sh2o(3) = state%sh2o(3)+eta(3)*0.0006

 state%smc(4)  = state%smc(4) +eta(4)*0.0004
 state%sh2o(4) = state%sh2o(4)+eta(4)*0.0004
